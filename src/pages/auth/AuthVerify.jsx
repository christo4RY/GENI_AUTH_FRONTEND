...
import { useUserVerifyCodeMutation } from "../../features/api/apiSlices/AuthApi";
import { useDispatch, useSelector } from "react-redux";
import { setCredentials } from "../../features/slices/auth/authTokenSlice";
const AuthVerify = () => {
    const {id} = useSelector((state)=>state.authToken)
    const dispatch = useDispatch()
    const [userVerifyCode, { isLoading }] = useUserVerifyCodeMutation()
    const form = useForm({
        initialValues: {id:"", verifycode: "" },
    });
    useEffect(()=>{
    ...
    const verify = async (e) => {
        e.preventDefault()
        const {data} = await userVerifyCode(form.values)
        console.log(data)
        if(data?.token){
            dispatch(setCredentials({id:data.id,token:data.token}))
            nav('/admin/products')
        }else{
            alert('invalid')
        }
            ...
            <div className="w-full md:w-[50%] flex items-center">
                <div className="w-full">
                    <div className="w-full md:w-[70%] mx-auto p-8 md:p-10">
                        ...
                        <form onSubmit={verify} className="w-full flex flex-col items-center justify-center">
                            <VerificationInput classNames="w-full" {...form.getInputProps('verifycode')} placeholder="_" validChars="0-9" inputProps={{ inputMode: "numeric", autoComplete: "one-time-code" }} />
